pub(crate) mod story {
    pub const FETCH: &str = r#"SELECT id, name
        | FROM stories
        | WHERE id = $1"#;

    pub const LIST: &str = r#"SELECT id, name
        | FROM stories
        | WHERE id >= $1
        | ORDER BY id
        | LIMIT $2"#;
...
pub(crate) mod task {
    pub const FETCH: &str = r#"SELECT id, story_id, name, status
        | FROM tasks
        | WHERE id = $1"#;

    pub const LIST: &str = r#"SELECT id, story_id, name, status
        | FROM tasks
        | WHERE story_id = $1
        | ORDER BY id LIMIT $2"#;
